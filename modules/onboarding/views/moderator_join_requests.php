...
<?php if (empty($requests)): ?>
    <div class="card">
        <div class="card-body">
            <p class="text-muted">No join requests for your batch yet.</p>
        </div>
    </div>
<?php else: ?>
    <div class="card">
                        ...
                        <tr>
                            ...
                            <td class="actions">
                                <?php if ($item['status'] === 'pending'): ?>
                                    <form method="POST" action="/moderator/join-requests/<?= (int) $item['id'] ?>/approve" class="table-action-form">
                                        <?= csrf_field() ?>
                                        <button type="submit" class="table-icon-btn is-success" title="Approve request" aria-label="Approve request">
                                            <?= ui_lucide_icon('check') ?>
                                        </button>
                                    </form>
                                    <form method="POST" action="/moderator/join-requests/<?= (int) $item['id'] ?>/reject" class="table-action-form">
                                        <?= csrf_field() ?>
